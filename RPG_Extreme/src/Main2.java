...
class Game {
    ...
    public static int Trap_Method(Character character) {
        if (character.Accessories.contains("DX")) {
            character.Now_Hp--;
        }
        else {
    ...
    public static void Play_Game_Method(Input_data Id, BufferedWriter bw , Character character , ArrayList<Monster> monsters  ,
                                        ArrayList<Box_Weapon> box_weapons , ArrayList<Box_Shield> box_shields ,ArrayList<Box_Accessories> box_accessories) throws IOException {
    ...
    public static void Run_Game() throws  IOException {
        ...
        // 캐릭터의 특성을 저장할 객체를 만든다. 이때 생성자를 통해서 캐릭터의 초기값을 미리 세팅한다.

        Game.Get_Start_Location_Method(Id , character); // 캐릭터의 시작지점 설정


        Game.Play_Game_Method(Id , bw , character , monsters , box_weapons , box_shields , box_accessories);
        // 실제로 게임을 실행하는 메서드를 호출한다.
